...
def main():
    ...
    try:
        # Producto demo
        prod, _ = get_or_create(db, Product,
            sku="LAB-001", name="Labial Mate", barcode_main="7501234567890",
            uom="unit", is_active=True)
        ...
        get_or_create(db, PriceListItem, price_list_id=pl.id, product_id=prod.id, price=Decimal('129.00'))

        # Stock inicial
        loc, _ = get_or_create(db, StockLocation, code="MAIN", name="Almacén Principal", is_active=True)
        get_or_create(db, StockQuant, product_id=prod.id, location_id=loc.id,
        ...
        print(f"Seed OK | product_id={prod.id} barcode=7501234567890 price_list_id={pl.id} location=MAIN")
    finally:
        db.close()

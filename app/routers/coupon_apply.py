...
from app.routers.coupon import ValidateBody, validate_coupon  # reutilizamos la validación oficial
...
def _fetch_order(con: sqlite3.Connection, order_id: int) -> Optional[Dict[str, Any]]:
    row = con.execute(
        """SELECT id, subtotal, discount_total, tax_total, total, status
                         FROM pos_order WHERE id=?""",
        (order_id,),
    ).fetchone()
    if not row:
        return None
...
def _recalc_subtotal(con: sqlite3.Connection, order_id: int) -> float:
    row = con.execute(
        """SELECT COALESCE(SUM(line_total),0) FROM pos_order_line WHERE order_id=?""", (order_id,)
    ).fetchone()
...
def apply_coupons(body: ApplyCouponsBody):
    ...
    try:
        ...
        rowc = con.execute(
            "SELECT id FROM coupon WHERE UPPER(code)=UPPER(?)", (chosen["code"],)
        ).fetchone()
        coupon_id = rowc[0] if rowc else None

        con.execute(
            """INSERT INTO pos_order_coupon (order_id, coupon_id, code_snapshot, value_applied)
                       VALUES (?,?,?,?)""",
        ...
        lines = con.execute(
            """SELECT id, product_id, qty, unit_price, discount, line_total
                               FROM pos_order_line WHERE order_id=?""",
            (body.order_id,),
        ).fetchall()
        coupons = con.execute(
            """SELECT id, order_id, coupon_id, code_snapshot, value_applied
                                 FROM pos_order_coupon WHERE order_id=?""",
            (body.order_id,),
        ).fetchall()

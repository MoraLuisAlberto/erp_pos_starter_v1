...
def pay_discounted(
    ...
    if base_total is None:
        raise HTTPException(status_code=422, detail="base_total/amount/splits required to compute total")
    ...
    if payload.coupon_code:
        if payload.customer_id is None:
            raise HTTPException(status_code=422, detail="customer_id required when coupon_code is present")
    ...
    if sum_splits != expected_total:
        raise HTTPException(status_code=422, detail=f"splits_total_mismatch: got {sum_splits}, expected {expected_total}")

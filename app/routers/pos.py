...
def create_order_draft(
    payload: DraftOrderRequest, x_idempotency_key: str | None = Header(default=None)
):
    ...
    try:
        ...
        for it in payload.items:
            pli = (
                ...
                .filter_by(price_list_id=payload.price_list_id, product_id=it.product_id)
                .first()
            )
            if not pli:
                raise HTTPException(
                    status_code=422,
                    detail=f"Producto {it.product_id} sin precio en lista {payload.price_list_id}",
                )

            # Política de stock (V1 bypass)
            if (
                POLICY in ("warn", "enforce")
                and StockQuant is not None
                and StockLocation is not None
            ):
                pass
        ...
        o = PosOrder(
            ...
            total=subtotal,
            status="draft",
            undo_until_at=datetime.utcnow() + timedelta(seconds=5),
...
def pay_order(payload: PayRequest, x_idempotency_key: str | None = Header(default=None)):
    ...
    try:
        ...
        if x_idempotency_key:
            dup = (
                db.query(PosPayment)
                .filter_by(idempotency_key=x_idempotency_key, order_id=o.id)
                .first()
            )
            if dup:
                lines = db.query(PosOrderLine).filter_by(order_id=o.id).all()
                ...
                return {
                    "order": _serialize_order(o, lines),
                    "payment_id": dup.id,
                    "method": dup.method,
                    "amount": float(dup.amount),
                    "splits": [{"method": s.method, "amount": float(s.amount)} for s in splits],
                }
        ...
        if o.status != "draft":
            if o.status == "paid":
                ...
                return {
                    ...
                    "amount": (float(p.amount) if p else 0.0),
        ...
        if round(wallet_req + sum_splits, 2) != round(total, 2):
            raise HTTPException(
                422, f"wallet_apply({wallet_req}) + splits({sum_splits}) != total({total})"
            )
        ...
        if wallet_req > 1e-9:
            ...
            _wallet_redeem(
                db, payload.customer_id, wallet_req, o.id, x_idempotency_key, by_user="demo"
            )

        method = (
            "mixed" if (len(payload.splits) > 1 or wallet_req > 0) else payload.splits[0].method
        )
        p = PosPayment(
            order_id=o.id,
            method=method,
...
def undo_order(payload: UndoRequest):
    ...
    try:
        ...
        if o.status != "draft":
            raise HTTPException(
                409, f"UNDO soportado en MVP sólo para órdenes 'draft' (actual: {o.status})"
            )

import os, random, requests, concurrent.futures as cf
...
def test_concurrent_pay_same_idempotency_key():
    ...
    sid = _session_id(js); assert st==200 and sid
    ...
    st, js = _post("/pos/order/draft", draft); assert st==200 and "order_id" in js
    oid = js["order_id"]
    ...
    def do_pay():
        r = requests.post(f"{BASE}/pos/order/pay-discounted", json=body, headers=headers, timeout=15)

...
def main():
    ...
    try:
        ...
        row = s.execute(
            text(
                "SELECT id, stage, kind, total, by_user FROM cash_count WHERE session_id=:sid ORDER BY id DESC LIMIT 1"
            ),
            {"sid": sid},
        ).fetchone()
        print(
            f"CASH_PRE_OK id={row[0]} stage={row[1]} kind={row[2]} total={row[3]} by_user={row[4]}"
        )
    finally:
        s.close()

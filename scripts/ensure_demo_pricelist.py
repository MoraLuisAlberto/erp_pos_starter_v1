...
def notnull_cols_without_default(cols):
    return [
        c for c in cols if c[3] == 1 and (c[4] is None) and c[5] == 0
    ]  # NOT NULL, sin default y no PK
...
def main():
    ...
    try:
        ...
        pli_tbl = pick_first_existing(
            s, ["price_list_item", "pricelist_item", "price_list_items", "pricelist_items"]
        )
        ...
        if not pricelist_tbl:
            ...
            s.execute(
                text("CREATE TABLE IF NOT EXISTS pricelist (id INTEGER PRIMARY KEY, name TEXT)")
            )
            s.commit()
        ...
        list_col = (
            "price_list_id"
            if has_col(pli_cols, "price_list_id")
            else ("pricelist_id" if has_col(pli_cols, "pricelist_id") else "list_id")
        )
        prod_col = (
            "product_id"
            if has_col(pli_cols, "product_id")
            else ("prod_id" if has_col(pli_cols, "prod_id") else None)
        )
        price_col = (
            "price"
            if has_col(pli_cols, "price")
            else ("unit_price" if has_col(pli_cols, "unit_price") else "amount")
        )
        if not prod_col:
            raise SystemExit(f"ERROR: No se encontró columna de producto en {pli_tbl}")

        row = s.execute(
            text(f"SELECT id, {price_col} FROM {pli_tbl} WHERE {list_col}=:l AND {prod_col}=:p"),
            {"l": 1, "p": 1},
        ).fetchone()
        if row:
            s.execute(
                text(f"UPDATE {pli_tbl} SET {price_col}=:pr WHERE id=:id"),
                {"pr": 129.00, "id": row[0]},
            )
            s.commit()
            pli_state = "updated"
            ...
            s.execute(
                text(
                    f"INSERT INTO {pli_tbl} ({list_col}, {prod_col}, {price_col}) VALUES (:l,:p,:pr)"
                ),
                {"l": 1, "p": 1, "pr": 129.00},
            )
            s.commit()
        ...
        try:
            s.execute(
                text(
                    f"CREATE UNIQUE INDEX IF NOT EXISTS ux_{pli_tbl}_lp ON {pli_tbl}({list_col}, {prod_col})"
                )
            )
            s.commit()
        except Exception:
        ...
        print(
            f"SEEDED_PLI      -> table={pli_tbl} list_col={list_col} prod_col={prod_col} price_col={price_col} state={pli_state} price=129.00"
        )
    finally:
        s.close()

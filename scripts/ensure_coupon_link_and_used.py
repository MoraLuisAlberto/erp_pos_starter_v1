...
def main():
    ...
    try:
        ...
        if not cid:
            ...
            data = {"code": code}
            if "type" in names: data["type"] = "percent"
            if "percent" in names: data["percent"] = 10
            if "used_count" in names: data["used_count"] = 0
            if "active" in names: data["active"] = 1
            ...
            for (_, name, ctype, notnull, dflt, pk) in cols:
                if notnull == 1 and pk == 0 and dflt is None and name not in data and name != "id":
                    ...
                    if "int" in ctype: data[name] = 0
                    elif any(k in ctype for k in ("real","floa","doub","num","dec")): data[name] = 0.0
                    else: data[name] = ""
        ...
        """))
        s.execute(text("INSERT OR IGNORE INTO coupon_audit(coupon_id,event,notes) VALUES (:c,'used',:n)"),
                  {"c": cid, "n": f"order:{oid}"})

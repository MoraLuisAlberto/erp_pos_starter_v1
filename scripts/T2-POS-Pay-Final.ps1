...
Write-Host "== T2 POS: session -> draft -> pay (splits, idempotente) =="
...
# 3) Pagar con splits (requisito del backend)
$K = ([guid]::NewGuid().ToString("N").Substring(0,12))
$headers = @{ "x-idempotency-key" = $K }

$payBodies = @(
  @{ order_id = $orderId; session_id = $sid; splits = @(@{ method = "cash"; amount = $total }) }
  # Si el backend exige montos "received" y "change", prueba esta segunda variante:
  # , @{ order_id = $orderId; session_id = $sid; splits = @(@{ method = "cash"; amount = $total; received = $total; change = 0 }) }
)

Write-Host ("`n-- POST /pos/order/pay (K={0}) con splits" -f $K)
$pay1 = $null; $picked = $null
foreach($b in $payBodies){
  Write-Host ("Pay body: " + (J $b))
  $r = POSTV "$BaseUrl/pos/order/pay" $b $headers
  if ($r) { $pay1 = $r; $picked = $b; break }
}

if (-not $pay1) { Write-Error "Pay intento 1 falló (ver WARNING con JSON de error)."; exit 5 }
$pay1 | Out-String | Write-Host

Write-Host ("`n-- Reintento /pos/order/pay (K={0}) idempotente" -f $K)
$pay2 = POSTV "$BaseUrl/pos/order/pay" $picked $headers
...
$pid1 = Extract-Int $pay1 @('payment_id')
$pid2 = Extract-Int $pay2 @('payment_id')
...
if ($oid1 -and $oid2 -and ($oid1 -eq $oid2)) {
  ...
  } elseif ($pid1 -and $pid2 -and ($pid1 -eq $pid2)) {
    Write-Host (">> OK: Idempotencia preservada (order_id={0}, payment_id={1})." -f $oid1,$pid1); exit 0
  } else {
    Write-Host (">> OK: Idempotencia preservada (order_id={0})." -f $oid1); exit 0
  }
} else {
  Write-Host (">> ALERTA: order_id inconsistente (1:{0}, 2:{1})." -f $oid1,$oid2); exit 7
}

...
if not table_exists("product_barcode"):
    cur.execute("""CREATE TABLE product_barcode (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    if "code" not in c:
        ...
        if "barcode" in c:
            ...
            cur.execute("""UPDATE product_barcode
...
try: cur.execute("CREATE UNIQUE INDEX ix_product_barcode_code ON product_barcode(code)")
except sqlite3.OperationalError: pass
try: cur.execute("CREATE INDEX ix_product_barcode_product ON product_barcode(product_id)")
except sqlite3.OperationalError: pass
...
if not table_exists("price_list_item"):
    cur.execute("""CREATE TABLE price_list_item (
        id INTEGER PRIMARY KEY AUTOINCREMENT,

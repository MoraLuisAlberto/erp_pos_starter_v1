...
if not table_exists("store"):
    cur.execute("CREATE TABLE store(id INTEGER PRIMARY KEY AUTOINCREMENT, code VARCHAR(40) UNIQUE, name VARCHAR(120) NOT NULL)")
...
if not table_exists("pos_session"):
    cur.execute("""CREATE TABLE pos_session(
...
if row:
    ...
    cur.execute("INSERT INTO pos_session (status,opened_at,opened_by,store_id,note) VALUES (?,?,?,?,?)",

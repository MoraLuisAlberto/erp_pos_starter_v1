...
def table_exists(s,t):
    return bool(s.execute(text("SELECT 1 FROM sqlite_master WHERE type='table' AND name=:t"),{"t":t}).fetchone())
...
def main():
    ...
    try:
        ...
        try:
            ...
            print(f"STORE -> id={row[0]} name={row[1]}") if row else print("STORE -> (no table or no row)")
        except Exception:

...
def seed_welcome10(s):
    ...
    data = {"code": "WELCOME10"}
    # defaults “estándar”:
    if "type" in [c[1] for c in info]:       data["type"] = "percent"
    if "percent" in [c[1] for c in info]:    data["percent"] = 10
    if "used_count" in [c[1] for c in info]: data["used_count"] = 0
    if "active" in [c[1] for c in info]:     data["active"] = 1

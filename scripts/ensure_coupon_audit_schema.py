...
def trigger_exists(s, name):
    return bool(
        s.execute(
            text("SELECT name FROM sqlite_master WHERE type='trigger' AND name=:n"), {"n": name}
        ).fetchone()
    )
...
def main():
    ...
    try:
        ...
        s.execute(
            text(
                "UPDATE coupon_audit SET created_at=COALESCE(created_at, CURRENT_TIMESTAMP) WHERE created_at IS NULL"
            )
        )
        ...
        if not index_exists(s, "ux_coupon_audit_evt"):
            try:
                s.execute(
                    text(
                        "CREATE UNIQUE INDEX ux_coupon_audit_evt ON coupon_audit(coupon_id,event,notes)"
                    )
                )
        ...
        s.commit()
        print(
            f"SCHEMA_OK created_at={'ADDED' if added_created else 'EXISTING'} dups_removed={removed}"
        )
    finally:
        s.close()

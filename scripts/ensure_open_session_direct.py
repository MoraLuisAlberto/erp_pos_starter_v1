...
def main():
    ...
    try:
        ...
        s.execute(text("CREATE TABLE IF NOT EXISTS pos_terminal (id INTEGER PRIMARY KEY, store_id INTEGER, name TEXT)"))
        ...
        row = s.execute(text("SELECT id FROM pos_session WHERE status='open' ORDER BY id DESC LIMIT 1")).fetchone()
        if row:
